...
# 匹配所有的测试结果文件
matching_files = glob.glob("./yours/*.irp")
...
for file in matching_files:
    ...
    # 收集所有不同的行号和内容
    diff_lines = [
        (i + 1, file_contents[i], correct_file_contents[i])
        for i in range(min_len)
        if file_contents[i] != correct_file_contents[i]
    ]
    ...
    if not diff_lines:
        ...
        failed_msg = (
            ...
            + "\n\t"
            + "different line(s): "
            + ", ".join([str(ln) for ln, _, _ in diff_lines])
            + "\n"
            + "\n".join(
                [
                    f"\tLine {ln}: \n\tYou: '{y}' \n\tRef: '{c}'"
                    for ln, y, c in diff_lines
                ]
            )
        )
        pass_all = False
